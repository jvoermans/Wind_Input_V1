...
etl::vector<uint16_t, samples_per_channel_per_file + 1> pitch;
etl::vector<uint16_t, samples_per_channel_per_file + 1> roll;
...
void DataManager::gather_dataset(void){
    ...
    // range pitch roll: -360 to 360 (not sure if -180 180 or 0 360 or else is used, take no change as accuracy is more than enough)
    static constexpr float range_pitch_roll = 2.0 * 360.0;
    // offset pitch roll: 360, to make sure always positive
    static constexpr float offset_pitch_roll = 360.0;
    ...
    while(step < samples_per_channel_per_file){
        ...
        e_pitch = static_cast<uint16_t>((r_pitch + offset_pitch_roll) / range_pitch_roll * 65000.0f);
        e_roll = static_cast<uint16_t>((r_roll + offset_pitch_roll) / range_pitch_roll * 65000.0f);
        ...
        pitch.push_back(e_pitch);
        roll.push_back(e_roll);
